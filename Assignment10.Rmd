...
if(useF$Maple.Species=="Red Maple"){useF$color="red"}else
if(useF$Maple.Species=="Silver Maple"){useF$color="grey"}else
if(useF$Maple.Species=="Sugar Maple"){useF$color="yellow"}else
if(useF$Maple.Species=="Norway Maple"){useF$color="green"}else{useF$color="black"}
```

```{r}

useF=subset(useF,(Longitude>-80)&(Longitude<-60)&(Latitude>35)&(Latitude<50))
```

```{r}
unique(useF$Maple.Species)
```
...
gg <- gg + geom_map(data=canp, map=canp, 
                    aes(x=long, y=lat, map_id=id, group=group),
                    fill=NA, color="black")
gg <- gg + geom_point(data=useF, aes(x=Longitude, y=Latitude, color=useF$color), 
                      alpha=1, na.rm=TRUE)
