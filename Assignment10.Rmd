...
if(full1$Maple.Species=="Red Maple"){full1$color="red"}else
if(full1$Maple.Species=="Silver Maple"){full1$color="grey"}else
if(full1$Maple.Species=="Sugar Maple"){full1$color="yellow"}else
if(full1$Maple.Species=="Norway Maple"){full1$color="green"}else{useF$color="black"}
```
```{r}
full3=subset(full1,
             (Longitude>-85)
...
gg <- gg + geom_map(data=canp, map=canp, 
                    aes(x=long, y=lat, map_id=id, group=group),
                    fill=NA, color="black")
gg <- gg + geom_point(data=useF, aes(x=Longitude, y=Latitude, color=color), 
                      alpha=1, na.rm=TRUE)
